...
## we don't want to abuse so comment
#
#
#
...
# nb_proposals_this_month
# total_value_proposals_this_month
# nb_proposals_this_month_projection
# total_value_proposals_this_month_projection
# proposals_active
# {{proposal['start']}}
# {{proposal['end']}}
# {{proposal['ask']}}
...
#with open('config.json', 'r') as config_file:
    # config_data = json.load(config_file)
    # rpcuri = 'http://{}:{}'.format(config_data.get('rpchost'), config_data.get('rpcport'))
    # getinfo_request = '{"jsonrpc":"1.0","id":"curltext","method":"getinfo","params":[]}'
    # print(type(getinfo_request))
    # response = requests.get(rpcuri, headers={'content-type': 'application/json'}, data=getinfo_request, auth=(config_data.get('rpcusername'), config_data.get('rpcpassword')))
    ...
    # print(current_block)
    # with open('getinfo.txt', 'w') as outfile:
    #     json.dump(my_response, outfile)
    ...
    # # with open('nextsuperblock.txt', 'w') as outfile:
    # #     json.dump(nextsuperblock, outfile)
    ...
    # with open('budgetinfo.txt', 'w') as outfile:
    #     json.dump(budgetinfo, outfile)


 	#What I would like to see is a clear list of MN budget dealines (I.e. how long to vote on each proposal), next Superblock (with a countdown?), as well as where the allotted funds go.
